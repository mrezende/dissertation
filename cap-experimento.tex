...
Utilizamos os dados disponibilizados por \cite{yao-2018}, pois esses dados contém duas características importantes: A amostra contém questões do tipo \textit{how-to-do-it} e as questões foram coletadas de um site aberto de perguntas e respostas, onde as questões costumam expressar as intenções de busca do desenvolvedor. Segundo \cite{yao-2018}, as questões do tipo \textit{how-to-do-it} costumam ser mais diretas e ter apenas um trecho de código-fonte como solução. E diferente do trabalho de XXX e YYY, onde eles coletaram quest

ões do stackoverflow de qualquer categoria, as questoes do tipo how-to-do-it por conterem perguntas diretas, indicam com uma maior precisão a semântica do trecho de código-fonte, diminuindo os problemas de ZZZZ e WWWW. Já \cite{cambronero-deep-learning-code-search:2019} apontaram que as redes neurais treinadas com pares de questões e trechos de código-fonte coletadas do \Gls{sof} obtiveram um desempenho superior na recuperação de trecho de código-fonte em relação a modelos que treinaram apenas com pares de comentários \gls{docstring} e trechos de código do Github. O próprio pessoal do Github verificou em seus experimentos que os comentários \gls{docstring} não conseguiam refletir a intenção de busca do usuário na recuperação de trecho de código-fonte, acarretando em resultados insatisfatórios na tentativa de criação de uma busca semântica em seus repositórios \citep{husain-github-semantic-search-code-2019}. Para nós, um indicativo de maior precisão semântica e reflexo das intenções de busca do desenvolvedor nas questões coletadas ressaltam a importância do uso dos dados disponibilizados por \cite{yao-2018} como fonte de conhecimento para o treinamento dos nossos modelos. I'm typing then when I type a special character, then 
á it goes the next line. If i go to old line (lines saved), I cant type special character á

If i go to a new line, then I can see that I can type a special character áõ (it seems that it doenst break into new line). 
