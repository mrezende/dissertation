...
Conforme a Figura~\ref{fig:evaluation-process}, a cada época um modelo candidato é avaliado quanto a média MRR. Esta média MRR é calculada na amostra DEV, onde todos os pares $<q_{i}, c_{i}^{+}>$ da amostra, sendo $q_{i}$ uma questão e $c_{i}^{+}$ uma questão anotada como correta, são avaliados juntos com outros 49 distratores $c'$, que são selecionados aleatoriamente da amostra de treinamento, tal que $c_{i}^{+} \neq c'$. 


Para cada questão, o modelo calcula a similaridade entre a questão e os trechos de código-fonte. O cálculo de similaridade é feito através da função $h_{\theta}$, onde $h_{\theta}$ é a função \textit{cosine}. 
