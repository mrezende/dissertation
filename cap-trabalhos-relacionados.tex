...
\caption{Ilustração de vetores de representação esparsa e distribuída. }
\label{fig:discrete-vs-distributed-representation}
\end{figure}


FAZER UMA IMAGEM explicando a diferença como o site do oreilly. \url{https://www.oreilly.com/content/how-neural-networks-learn-distributed-representations/}.



No caso, o vetor de representação distribuída ou \textit{embedding} permite representar uma palavra através de um vetor com valores reais ($\mathbb{R}$). Podemos definir \textbf{embedding} como uma função $\mathbb{E}$ \citep{cambronero-deep-learning-code-search:2019}:
