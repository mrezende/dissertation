...
\begin{subfigure}{.7\textwidth}
  ...
  \caption{Ilustração da abordagem sequencial, onde os vetores são fornecidos individualmente à rede neural. A cada passo $t$, a rede neural \emph{A} combina um vetor da palavra $w_{t}$ com o vetor de resultado anterior $h_{t - 1}$. Ao final de $t$ passos, obtemos o vetor de representação da sentença $h_{t}$. Imagem baseada na figura do website: \url{https://colah.github.io/posts/2015-08-Understanding-LSTMs/}}
...
O intuito de combinar os vetores contínuos das palavras é sintetizar as principais características da sentença em um vetor. Além disso, queremos estabelecer relações semânticas, onde sentenças semanticamente similares sejam representadas por vetores similares. Com um rico espaço de similaridade, é possível ensinar a rede neural a correlacionar as questões aos trechos de código-fonte mais facilmente. Além disso, é possivel reutilizar estes vetores de representação em outras tarefas, onde o codigo -fonte pode ser recomendado, ou permitir a identificação de linguagens e etc. autocomplete, duvida, etc.
