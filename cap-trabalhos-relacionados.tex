...
\end{itemize}
 ...
 O \acrshort{cbow} é uma rede neural sequencial que mapeia uma matriz de inteiros para uma matriz de vetores contínuos. Conforme a Figura~\ref{fig:word2vec-cbow}, o \acrshort{cbow} aprende os coeficientes da matriz predizendo uma palavra-alvo a partir de um contexto de palavras ao redor. A partir disso, os ajustes dos coeficientes são realizados nos vetores contínuos das palavras do contexto. Ao final, temos uma matriz de vetores contínuos aprendidos, onde a ordem dos vetores para predizer uma palavra não importa, pois os vetores contínuos são somados antes de predizer a palavra-alvo (ver Figura
~\ref{fig:word2vec-cbow}).
 
 O \textit{Skip-gram} também é formado por uma rede neural sequencial, porém os coeficientes da matriz são aprendidos predizendo as palavras do contexto a partir de uma palavra-alvo, conforme a Figura~\ref{fig:word2vec-skip-gram}. Diferente do \acrshort{cbow}, o \textit{Skip-gram} realiza ajustes nos vetores contínuos das palavras-alvos.  Essa diferença no método de aprendizagem, resulta em vetores contínuos com diferentes características extraídas das palavras. De acordo com \cite{mikolov2013distributed}, o \acrshort{cbow} apresentou bons resultados para tarefas associadas a relações sintáticas das palavras, e.g., associar o adjetivo a advérbio, encontrar o antôninmo ou o superlativo de uma palavra. Enquanto o \textit{Skip-gram} apresentou melhores resultados para associações semânticas como encontrar a cidade de um estado, a moeda de um país ou relacionar palavras masculinas e femininas.
...
Para exemplificar o uso do \textit{word2vec}, suponha que tenhamos o trecho de código-fonte a seguir:
