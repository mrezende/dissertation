...
Na comparação indidividual, os pares de questões e trechos de código-fonte são comparados individualmente. Para um par $(q_{i}, c_{i})$ e a correspondente classificação $y_{i}$ anotada, onde $q_{i}$ é um vetor contínuo de uma questão e $c_{i}$ é um vetor contínuo de um trecho de código, podemos definir a função objetivo \textit{pointwise} como:
