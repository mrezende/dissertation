...
Conforme \cite{cambronero-deep-learning-code-search:2019}, uma boa parte dos modelos propostos para o problema de \textit{code retrieval} utilizam a abordagem \textit{join embedding}. \cite{Allamanis-bimodal-source-code-natural-language:2015} criaram um modelo probabilístico para recuperar o trecho de código-fonte mais relevante para uma determinada questão. Ele combinou os vetores de representação de código-fonte e descrição em linguagem natural através de uma operação multiplicativa.
...
\hline
\textbf{Artigo} & \textbf{Representação da palavra} 
& \textbf{Representação do texto} & \textbf{Ordem} \\
\hline
\cite{Allamanis-bimodal-source-code-natural-language:2015} & Representação distribuída / Árvore sintática & Média / Operação multiplicativa & Bag / Árvore sintática\\
\hline 
\cite{Chen-bi-variational-autoencoder:2018} & Representação distribuída & bi-\acrshort{vae} & Bag\\
 \hline
\cite{iyer-etal-2016-summarizing} & \gls{one-hot-encoding} & \acrshort{lstm} com \gls{mecanismo-atencao} & Sequencial\\
 
 \hline
 
 \cite{Gu-deep-code-search:2018} & \textit{word2vec} & bi-\acrshort{lstm} & Sequencial\\
 \hline
 
 \citep{Sachdev-neural-code-search:2018} & \textit{word2vec} & Média / \acrshort{tf-idf}  & Bag\\
 
 \hline
 
 \cite{cambronero-deep-learning-code-search:2019} & \textit{word2vec} & Média / \gls{mecanismo-atencao} & Bag \\
...
\cite{Allamanis-bimodal-source-code-natural-language:2015} -> preprocessing code: 
\cite{Chen-bi-variational-autoencoder:2018} -> 
\cite{iyer-etal-2016-summarizing} -> 
\cite{Gu-deep-code-search:2018} -> 
\citep{Sachdev-neural-code-search:2018} -> 
\cite{cambronero-deep-learning-code-search:2019} -> 
