...
A abordagem comumente utilizada ensina a rede neural a discriminar os trechos de códigos-fontes relevantes dos não relevantes a partir de uma questão. Para realizar essa discriminação, os trabalhos fazem uso da técnica de agrupamento de vetores contínuos ou \textit{joint embedding}. Essa técnica permite agrupar dados heterogêneos de forma que conceitos semanticamente similares ocupem regiões próximas em um mesmo espaço vetorial. A ilustração dessa técnica pode ser visualizada na Figura~\ref{fig:joint-embedding}.
...
\end{figure}
 ...
 Na Figura~\ref{fig:joint-embedding}, ilustramos a técnica \textit{joint embedding} para a recuperação de trecho de código-fonte. Há duas redes neurais, que funcionam .... uma para mapear as questões para um vetor contínuo e a outra para mapear os trechos de código-fonte. No nosso caso, estamos agrupando vetores de trechos de código-fonte que foram anotados como corretos às questões coletadas do \Gls{sof}. Para utilizar essa técnica, três itens devem ser levados em consideração:
