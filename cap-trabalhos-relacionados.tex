...
Conforme \cite{cambronero-deep-learning-code-search:2019}, uma boa parte dos modelos propostos para o problema de \textit{code retrieval} utilizam a abordagem \textit{joint embedding}. Na tabela~\ref{table:summary-joint-embedding}, compilamos os principais trabalhos que utilizam esta abordagem. 
...
& Combinou os vetores de \textit{tokens} de questão através da operação de média / Combinou os \textit{tokens} de código-fonte usando operações multiplicativas\\
...
O uso de redes neurais é um outro ponto em comum dos trabalhos reunidos na tabela~\ref{table:summary-joint-embedding}. Tanto que \cite{cambronero-deep-learning-code-search:2019} cunhou o termo \textit{neural code search}, o uso de redes neurais na recuperação de trecho de código-fonte. É interessante observar que nenhum trabalho verificou o desempenho das redes convolucionais neste problema. Sendo que as redes convolucionais apresentaram um bom desempenho em problemas similares em NLP \todo{citar trabalhos da YAO}.

Infelizmente, não há um consenso sobre o conjunto de dados para avaliação do modelo. Isto deve-se, principalmete, a falta de um conjunto de dados curado e organizado para este problema. 

sumário do modelo joint embedding:



Sumário do dataset:

\cite{Allamanis-bimodal-source-code-natural-language:2015} -> preprocessing code: syntax tree for source code, stackoverflow, title for questions, code snippet as code, C\#
\cite{Chen-bi-variational-autoencoder:2018} -> title for question, code snippet for code answer, C\#, SQL like \cite{iyer-etal-2016-summarizing}, avaliação mesma do \cite{iyer-etal-2016-summarizing}.
\cite{iyer-etal-2016-summarizing} -> stackoverflow, c# e SQL, title for question and code snippet as code answer, avalição feita com dados anotados manualmente.
\cite{Gu-deep-code-search:2018} -> method name, api sequence and tokens for code, docstring for description language, github, java, avaliação nas 50 questoes mais votadas do SoF
\citep{Sachdev-neural-code-search:2018} -> method name, api sequence, enums, string literals and comments for code, none for query. github for training and SoF for evaluation. Java
