...
O modelo que obtiver a maior média \emph{MRR} ao final do treinamento é escolhido.
...
Os resultados apresentados pelo modelo bi-LSTM com CNN e CNN parecem bastante promissores, a nosso ver. O próximo passo é comparar os nossos modelos com o modelo proposto por \cite{cambronero-deep-learning-code-search:2019} que é o estado da arte. 
...
Nas figuras \ref{fig:grafico-erro-bi-lstm-cnn} e \ref{fig:grafico-erro-cnn} abaixo, podemos ver que tanto bi-LSTM com CNN quanto o CNN apresentaram uma diferença grande entre a curva de erro na amostra de validação e da amostra de treinamento, principalmente a partir da época $5$ (eixo $X$), próximo da interrupção do treinamento. Um indicativo de \textit{overfitting}. Já no caso da arquitetura \textit{Embedding}, figura \ref{fig:grafico-embedding}, as duas curvas de erro estão próximas até a interrupção. 
