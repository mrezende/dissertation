...
O código a seguir é referente ao modelo que utilizamos como base para comparação em nossos experimentos. Este modelo é bem simples, ele recebe como entrada na primeira camada os vetores de representação distribuída das questões e trechos de código-fonte. E extrai as características mais relevantes dos vetores através de uma camada \textit{maxpool} (linha 22). E, ao final, a distância de similaridade \textit{cosine} é calculada entre os vetores de representação distribuída da questão e do trecho de código-fonte. 
